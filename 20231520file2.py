...
# [cite_start]목적: 상품명 받침 유무를 판별하여 "치킨이", "초밥을" 등 자연스러운 문장을 구성함 
...
# 2. 메인 화면 및 학습 목표 설정
# [cite_start]결과: 학생이 수업의 주제와 목표를 명확히 인지하도록 상단 제목 출력 [cite: 55, 106]
st.title("🛒 우리 집 '합리적 소비' 매니저")
st.subheader("합리적으로 선택해 보아요.")

# 3. 주제별 데이터 정의 (교과 내용 CK 연계)
# [cite_start]목적: 지도서의 경제 단원 내용을 바탕으로 실생활 주제와 평가 기준을 설정함 [cite: 59, 164]
THEMES = {
...
with col_a:
    ...
    # [cite_start]결과: 학생들이 주관적인 만족도를 슬라이더로 데이터화함 [cite: 106]
...
# [cite_start]목적: 수집된 데이터를 비교하여 최적의 대안을 제안하고 기회비용을 명시함 [cite: 59, 123]
if st.button("🤖 AI 매니저에게 합리성 분석 요청하기"):
    ...
    if price_a > budget and price_b > budget:
        ...
        # 기회비용 분석: 선택하지 않은 쪽이 더 우수했던 순수 항목명 리스트 추출
        lost_advantages = [k for k, v in o_eval.items() if v > b_eval[k]]
        
        if lost_advantages:
            # [cite_start]결과: 포기한 항목들을 자연스러운 문장으로 결합하여 출력 [cite: 59]
            adv_text = " 및 ".join([f"**{a}**" for a in lost_advantages])
            st.warning(f"💡 **기회비용 확인:** {get_josa(best, '을/를')} 선택하면 {get_josa(other, '이/가')} 가진 {adv_text}{get_josa(lost_advantages[-1], '을/를')} 포기하게 됩니다.")
        else:
            st.warning(f"💡 **기회비용 확인:** {get_josa(best, '을/를')} 선택하면 {other}라는 대안 자체를 포기하게 됩니다.")
            
        # [cite_start]결과: AI 윤리 및 주체적 의사결정 태도 강조 [cite: 40, 145]
